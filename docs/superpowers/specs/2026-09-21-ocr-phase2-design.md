...
## 残タスク(次フェーズ)
- **モデルの自前配布 — 完了**(フェーズ3。詳細は [フェーズ3計画](../plans/2026-09-21-phase3-model-hosting.md)): 取得元をNDL公式 `ndl-lab/ndlocr-lite` の固定コミットへ切替+SHA-256検証、Cloudflare R2 非公開バケット `ebook-maker` へのミラー配置(5オブジェクト)、公式が全滅したときだけ発動するCIの自動フォールバック(SigV4署名のS3互換API経由。`wrangler` は所有者の手動アップロード専用)、取得元の週次死活監視(公式が全滅すればR2が代替していても警報)まで実装済み。上流作者の個人バケットへの依存は解消した。バケット名・トークン権限・モデル差し替え時のミラー再実行手順といった運用上の詳細は [Cloudflare R2 運用手順](../../ops/cloudflare-r2.md) にある。
