...
## 残タスク(次フェーズ)
- **モデルの自前配布**(一部完了: 取得元のNDL公式固定コミットへの切替とSHA-256検証、R2ミラー用ツール `npm run mirror-models` は完了。R2バケット作成とCIへの組み込み(フォールバック・死活確認)が残り。詳細は [フェーズ3計画](../plans/2026-09-21-phase3-model-hosting.md)): NDLOCR-Lite 本家からモデル(DEIMv2・PARSeq 3種)を自分で取得・変換し、自前のR2バケット等に置いて配布元を切り替える。現状は上流作者の個人バケットに依存しており、消えるとデプロイが壊れる。作業には、取得手順の確立、`scripts/modelFiles.mjs` のURL/SHA-256差し替え、ライセンス・帰属表示の見直しが含まれる。
