...
## 残タスク(次フェーズ)
- **モデルの自前配布**: NDLOCR-Lite 本家からモデル(DEIMv2・PARSeq 3種)を自分で取得・変換し、自前のR2バケット等に置いて配布元を切り替える。現状は上流作者の個人バケットに依存しており、消えるとデプロイが壊れる。作業には、取得手順の確立、`scripts/modelFiles.mjs` のURL/SHA-256差し替え、ライセンス・帰属表示の見直しが含まれる。
