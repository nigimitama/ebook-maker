...
| 7127 | U+20BB7 | U+3013 |
  ...
  現行版は U+3013 が計4回出現し重複を持つ(公式は重複なし)。文字集合の差は「公式のみ: 上記4文字 / 現行のみ: U+3013」。語彙サイズ 7142 = 7141 + EOS はどちらでも整合する。3・4行目以外(`charset_test` 行を除く他の行)に差は無い。どちらを採用するかは Task 1 で確定する。
