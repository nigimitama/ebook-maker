...
さらに、公式ソースが消えた場合に備えた**第2の経路**がある: 自前の Cloudflare R2 非公開バケット `ebook-maker` にモデル5本をミラーしてあり、公式が全滅したときだけ `fetch-models` が自動でフォールバックする(SigV4署名したS3互換API経由。取得後は公式と同じSHA-256で検証するため、ミラーが改変・陳腐化していれば失敗する)。公開バケットではないので実行時の別オリジン取得は発生しない。取得元の生死は週次ワークフローで監視し、公式ソースが落ちたらR2が代替できていても警報が鳴る。運用は [Cloudflare R2 運用手順](../../ops/cloudflare-r2.md) を参照。
