...
## 実施状況(2026-09-22 時点)
案A(公式固定コミットからのビルド時取得)と案B(R2 非公開バケットのミラー)の両方が稼働している。

- 一次ソース: NDL公式 `ndl-lab/ndlocr-lite` の固定コミット。`fetch-models` が `sources` を順に試す。
- 二次ソース: Cloudflare R2 の非公開バケット **`ebook-maker`** にモデル5本をアップロード済み(内容アドレスキー `models/<sha256>/<basename>`)。
- **CIからの取得はSigV4署名したS3互換API呼び出し**で行う。`wrangler` はCIでは使わない(CI用のSecretsはS3互換APIのアクセスキー対であり、`wrangler` はOAuthセッション前提で非対話認証できない)。`wrangler` は所有者がローカルで手動アップロードする経路にのみ残る。署名は `node:crypto` だけで自前実装し、npm依存は追加していない。
- フォールバックは自動だが**公式が全滅したときだけ**発動し、ミラー経路でも同じSHA-256検証を通す。Secretsが無い環境では公式のみで動作する(フォークのCIが落ちない)。
- 週次ワークフロー(`verify-models.yml`)が全取得元を実取得して死活監視する。**公式ソースが全滅した場合はR2が代替できていても非0終了+`::error::`**(早期警告の目的を守るため)。

運用手順・バケット名・トークン権限は [Cloudflare R2 運用手順](../../ops/cloudflare-r2.md) を参照。
案C(R2の実行時公開配信)は依然として不要(YAGNI)のまま。
