...
- 章立て(任意の4番目の工程): 目次ページのOCRから章名と開始ページを読み取り、PDFのしおり・EPUBの目次に埋め込む
- タイトル・著者のメタデータ入力
- PDF / EPUB(固定レイアウト・画像埋め込み型)の書き出し

操作は3つの必須工程(1. 読み込み → 2. 並べ替え・調整 → 詳細&書き出し)と、任意のOCR・章立て工程で進みます。
...
- 工程は「取込 → 調整 → OCR(任意) → 章立て(任意) → 書き出し」。OCRは飛ばしても従来どおり書き出せます。
